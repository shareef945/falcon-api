...
router.patch("/customer/:id", getCustomerInfo, async (req, res) => {
  ...
  if (req.body["Username"] != null) {
    res.customerInfo["Username"] = req.body["Username"];
  }
  if (req.body["Password"] != null) {
    res.customerInfo["Password"] = req.body["Password"];
  }
...
router.patch("/products/:id", getProductData, async (req, res) => {
  ...
  if (req.body["Preffered Payment Details"] != null) {
    res.productData["Preffered Payment Details"] =
      req.body["Preffered Payment Details"];
  }
  ...
  try {
    const updatedProductData = await res.productInfo.save();
...
async function getProductData(req, res, next) {
  let productData;
  try {
    productData = await ProductData.find({
      "Product ID": { $all: [req.params.id] },
    });
